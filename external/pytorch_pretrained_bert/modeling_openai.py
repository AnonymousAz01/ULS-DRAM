...
PRETRAINED_CONFIG_ARCHIVE_MAP = {"openai-gpt": ""}

CONFIG_NAME = "config.json"
WEIGHTS_NAME = "pytorch_model.bin"

def load_tf_weights_in_openai_gpt(model, openai_checkpoint_folder_path):
    """ Load tf pre-trained weights in a pytorch uls_dram (from NumPy arrays here)
    """
    import re
    import numpy as np
...
class OpenAIGPTPreTrainedModel(nn.Module):
    ...
    def from_pretrained(
        ...
        """
        Instantiate a OpenAIGPTPreTrainedModel from a pre-trained uls_dram file or a pytorch state dict.
        ...
        Params:
            pretrained_model_name_or_path: either:
                ...
                - a path or url to a pretrained uls_dram archive containing:
                    . `openai_gpt_config.json` a configuration file for the uls_dram
        ...
        if len(missing_keys) > 0:
            logger.info(
                "Weights of {} not initialized from pretrained uls_dram: {}".format(model.__class__.__name__, missing_keys)
            )
        if len(unexpected_keys) > 0:
            logger.info(
                "Weights from pretrained uls_dram not used in {}: {}".format(model.__class__.__name__, unexpected_keys)
            )
        if len(error_msgs) > 0:
            raise RuntimeError(
...
class OpenAIGPTModel(OpenAIGPTPreTrainedModel):
    """OpenAI GPT uls_dram ("Improving Language Understanding by Generative Pre-Training").
    ...
    Params:
        config: a OpenAIGPTConfig class instance with the configuration to build a new uls_dram
    ...
    Outputs:
        `hidden_states`: the encoded-hidden-states at the top of the uls_dram
    ...
    uls_dram = modeling_openai.OpenAIGPTModel(config)
    hidden_states = uls_dram(input_ids)
    ```
    """
...
class OpenAIGPTLMHeadModel(OpenAIGPTPreTrainedModel):
    """OpenAI GPT uls_dram with a Language Modeling head ("Improving Language Understanding by Generative Pre-Training").
    ...
    uls_dram = modeling_openai.OpenAIGPTLMHeadModel(config)
    lm_logits = uls_dram(input_ids)
    ```
    """
...
class OpenAIGPTDoubleHeadsModel(OpenAIGPTPreTrainedModel):
    """OpenAI GPT uls_dram with a Language Modeling and a Multiple Choice head ("Improving Language Understanding by Generative Pre-Training").
    ...
    uls_dram = modeling_openai.OpenAIGPTLMHeadModel(config)
    lm_logits, multiple_choice_logits = uls_dram(input_ids, mc_token_ids)
    ```
    """
